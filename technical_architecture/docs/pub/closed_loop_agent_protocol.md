...
> **New in v1.1.0**: Domain-Specific Behaviors via Strategy Pattern - See [Section 15](#15-domain-specific-behaviors-strategy-pattern) for bat-specific parsing and acoustic profiles.
...
15. [Domain-Specific Behaviors (Strategy Pattern)](#15-domain-specific-behaviors-strategy-pattern)
...
## 15. Domain-Specific Behaviors (Strategy Pattern)

### 15.1 Overview

The Closed-Loop Interaction Agent now supports domain-specific behaviors through the **Strategy Pattern**, enabling species-specific parsing and acoustic processing while maintaining backwards compatibility with the original general-purpose architecture.

**Key Benefits:**
- **Pluggable Behaviors**: Switch between compositional and holophrastic parsing
- **Species Optimization**: Tailored acoustic profiles for different species
- **Backwards Compatibility**: Default behavior unchanged for existing deployments
- **Test-Driven Development**: Dual-mode verification ensures no regressions
...
│   (Logic Layer)     │                    │  (Execution Layer)  │
├─────────────────────┤                    ├─────────────────────┤
│ ParsingStrategy     │                    │ AcousticProfile     │
│ ┌─────────────────┐ │                    │ ┌─────────────────┐ │
│ │Compositional    │ │                    │ │GeneralProfile   │ │
│ │(default)        │ │                    │ │(default)        │ │
│ └─────────────────┘ │                    │ └─────────────────┘ │
│ ┌─────────────────┐ │                    │ ┌─────────────────┐ │
│ │Holophrastic     │ │                    │ │BatProfile       │ │
│ │(bat-specific)   │ │                    │ │(bat-specific)   │ │
│ └─────────────────┘ │                    │ └─────────────────┘ │
└─────────────────────┘                    └─────────────────────┘
```

### 15.3 Python Logic Layer: Parsing Strategy

#### 15.3.1 Strategy Interface
...
class ParsingStrategy(ABC):
    ...
    def is_holophrastic(self) -> bool:
        """Return True if strategy uses holophrastic (idiom-based) parsing."""
        pass
```

#### 15.3.2 CompositionalStrategy (Default)

**Original behavior**: Each segment is an independent semantic unit.

```python
class CompositionalStrategy(ParsingStrategy):
    """Original behavior: segments = words"""
        ...
        return ParseResult(
            tokens=[
                ParsedToken(
                    ...
                    segments=[s],
                    meaning=self._lookup_segment(s),
                    confidence=1.0,
                )
                for s in segment_sequence
            ],
            strategy_used="compositional",
        )
```

#### 15.3.3 HolophrasticStrategy (Bat-Specific)

**Bat-specific behavior**: Rigid idioms are atomic units.

Based on Egyptian Fruit Bat Phase 2/3 research:
- **LRN-6 Idiom**: `[114, 464, 604, 324, 94, 714]` is unbreakable
- **Valid Bigrams**: Only 50 out of 260,100 possible (0.02%)
- **Position Specialists**: Openers (position 0), Closers (position 1)
...
class HolophrasticStrategy(ParsingStrategy):
    ...
    OPENERS = {384, 264, 1014, 1004, 534, 434, 514, 764}
    CLOSERS = {444, 304, 544, 404, 394, 684, 504}
...
class TokenType(Enum):
    COMPOSITIONAL = "compositional"  # Original: segment = word
    IDIOM = "idiom"                  # Bat: rigid pattern = single meaning
    CONTENT = "content"              # Bat: individual segment in known position
    NOISE = "noise"                  # Bat: segment not in known patterns
    OPENER = "opener"                # Bat: position 0 (staccato alert)
    CLOSER = "closer"                # Bat: position 1 (clean termination)
```

#### 15.3.5 Configuration
...
class InteractionAgentConfig:
    ...
    # Domain mode for Strategy Pattern
    # "general" = CompositionalStrategy (default)
    # "bat" = HolophrasticStrategy
    domain_mode: str = "general"

    # Optional segment meanings for parsing
    segment_meanings: Optional[Dict[int, str]] = None

    # Custom idioms for holophrastic mode
...
### 15.4 Rust Execution Layer: Acoustic Profile

#### 15.4.1 Profile Trait
...
#### 15.4.2 GeneralProfile (Default)

**Original behavior**: All transitions valid, uniform weights.
...
impl AcousticProfile for GeneralProfile {
    fn position_weights(&self) -> &PositionWeights { &self.weights }
    fn transition_strictness(&self) -> f32 { 0.0 }  // Permissive
    fn is_valid_transition(&self, _: usize, _: usize) -> bool { true }
    fn name(&self) -> &str { "general" }
}
```

#### 15.4.3 BatProfile (Bat-Specific)

**Bat-specific behavior**: Position-weighted logic with restrictive grammar.

Based on Phase 2/3 research:
- **Transition Strictness**: 0.98 (only ~0.02% of transitions valid)
- **Valid Bigrams**: 50 observed patterns
- **Position-Weighted Features**:
  - Openers: temporal=2.0, micro_dynamics=2.5 (staccato alerts)
  - Closers: harmonic=2.5, spectral=1.8 (clean termination)
        ...
        Self {
            weights: PositionWeights {
                ...
                opener: FeatureWeights {
                    temporal: 2.0,       // Duration critical
                    micro_dynamics: 2.5, // Energy/attack critical
                    harmonic: 0.8,       // HNR less important
                    overrides: vec![
                ...
                closer: FeatureWeights {
                    harmonic: 2.5,       // HNR CRITICAL
                    spectral: 1.8,       // Higher frequency
                    overrides: vec![
                        (6, 2.5),  // D6: harmonicity - CRITICAL
                        (7, 2.0),  // D7: harmonic_to_noise
                    ],
                },
                content: FeatureWeights::bat(),
            },
            valid_bigrams: vec![
                (764, 304), (534, 434), (304, 394),
                // ... 50 valid bigrams from Phase 2 research
            ].into_iter().collect(),
        }
    }
}
...
impl AcousticProfile for BatProfile {
    fn transition_strictness(&self) -> f32 { 0.98 }  // Restrictive
    fn is_valid_transition(&self, from: usize, to: usize) -> bool {
        self.valid_bigrams.contains(&(from, to))
    }
    fn name(&self) -> &str { "bat" }
}
```

### 15.5 Species Integration

#### 15.5.1 Python Factory

```python
class ParsingStrategyFactory:
    @staticmethod
    def create(domain_mode: str = "general", ...) -> ParsingStrategy:
        if domain_mode.lower() in ("bat", "holophrastic"):
            return HolophrasticStrategy(...)
        return CompositionalStrategy(...)
```
...
impl AcousticProfileFactory {
    pub fn create(species: &str) -> Box<dyn AcousticProfile> {
        match species.to_lowercase().as_str() {
            "egyptian fruit bat" | "bat" | "fruit_bat" => {
                Box::new(BatProfile::default())
            }
...
#### 15.5.3 SpeciesConfig Integration

```rust
impl SpeciesConfig {
    /// Create acoustic profile for this species
    pub fn create_acoustic_profile(&self) -> Box<dyn AcousticProfile> {
        AcousticProfileFactory::create(&self.species)
    }

    /// Get domain mode string for this species
    pub fn domain_mode(&self) -> &'static str {
        match self.species.to_lowercase().as_str() {
            "egyptian fruit bat" | "bat" | "fruit_bat" => "bat",
            _ => "general",
        }
    }
}
...
#### 15.6.1 General Mode (Default)

```python
# Default configuration - original behavior preserved
...
config = InteractionAgentConfig(
    domain_mode="bat",
    segment_meanings={114: "alert", 464: "query"},
)

agent = InteractionAgent(config=config)
```

#### 15.6.3 Custom Idioms

```python
# Add custom idioms for holophrastic mode
config = InteractionAgentConfig(
    domain_mode="bat",
    custom_idioms=[
        ([100, 200, 300], "CUSTOM_IDIOM_A", 0.95),
        ([400, 500], "CUSTOM_IDIOM_B", 0.90),
    ],
)
```

### 15.7 Test Verification (TDD)

#### 15.7.1 Python Tests

```bash
# Run parsing strategy tests
python3 -m pytest tests/test_parsing_strategy.py -v
...
|------|-------------|-------------------|
| `test_general_mode_fallback` | Test 1.1.1 | Each segment = independent token |
| `test_bat_mode_activation_lrn6` | Test 1.2.1 | LRN-6 collapses to single IDIOM |
| `test_context_isolation` | Test 1.3.1 | General mode ignores position |
| `test_original_behavior_preserved` | Backwards compat | 6 tokens from LRN-6, not 1 |

#### 15.7.2 Rust Tests

```bash
# Run acoustic profile tests
cargo test acoustic_profile --lib
...
| Test | Description | Expected Behavior |
|------|-------------|-------------------|
| `test_general_profile_execution` | Test 2.1.1 | All transitions valid, strictness=0.0 |
| `test_bat_profile_positional_weighting` | Test 2.2.1 | Opener weights ≠ Closer weights |
| `test_permissive_vs_restrictive` | Test 2.3.1 | [999,888] valid in general, rejected in bat |

### 15.8 Background Research Summary

#### 15.8.1 Phase 2 Findings (Linguistic Analysis)

| Finding | Value | Implication |
|---------|-------|-------------|
| Valid bigrams | 50 out of 260,100 possible | 0.02% combinatorial ratio |
| LRN-6 pattern | `[114, 464, 604, 324, 94, 714]` | Unbreakable rigid idiom |
| Function words | 0 detected | No compositional grammar |
| Context-specific segments | 0 | Context from external factors |

**Diagnosis**: FIXED-PATTERN, HOLOPHRASTIC communication system.

#### 15.8.2 Phase 3 Findings (Acoustic Archetype Mapping)

| Property | Openers | Closers | Expected | Observed |
|----------|---------|---------|----------|----------|
| Frequency | 5.33 kHz | 6.31 kHz | O > C | **REVERSED** |
| Duration | 31.6 ms | 58.0 ms | O < C | **CONFIRMED** |
| Energy | 9.50 | 7.04 | O > C | **CONFIRMED** |
| HNR | 32.9 | 80.6 | O < C | **CONFIRMED** |

**Conclusion**: Temporal framing is key - Openers are "staccato alerts", Closers are "clean termination".

### 15.9 Backwards Compatibility Guarantees

| Guarantee | Implementation |
|-----------|----------------|
| Default is General | `domain_mode="general"` if not specified |
| Rust Default is General | Unknown species get `GeneralProfile` |
| No API Changes | Existing `InteractionAgent` constructor unchanged |
| Test Verification | Tests 1.1.1 and 2.1.1 verify original behavior |

### 15.10 File Locations

| File | Layer | Description |
|------|-------|-------------|
| `src/realtime/parsing_strategy.py` | Python | ParsingStrategy interface and implementations |
| `src/realtime/interaction_agent.py` | Python | Integration with InteractionAgent |
| `technical_architecture/src/acoustic_profile.rs` | Rust | AcousticProfile trait and implementations |
| `technical_architecture/src/species.rs` | Rust | SpeciesConfig integration |
| `tests/test_parsing_strategy.py` | Test | Python TDD tests (38 tests) |
| `technical_architecture/src/acoustic_profile.rs` (inline) | Test | Rust TDD tests (18 tests) |
...
|---------|------|--------|---------|
| 1.1.0 | 2026-03-09 | Sheel Morjaria | Added Section 15: Domain-Specific Behaviors (Strategy Pattern) for bat-specific parsing and acoustic profiles |
