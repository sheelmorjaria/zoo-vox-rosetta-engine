...
impl PhraseSequenceAnalyzer {
    ...
    fn compute_spectrum(&self, audio: &[f32]) -> Vec<f64> {
        let n = audio.len().next_power_of_two().max(1024);
        let mut spectrum = vec![0.0f64; n / 2];
        let mut real = vec![0.0f64; n];
        let _imag = vec![0.0f64; n];
        ...
        for k in 0..n/2 {
            ...
            for t in 0..n {
                let angle: f64 = two_pi_n * k as f64 * t as f64;
                sum_real += real[t] * angle.cos();
                sum_imag += real[t] * angle.sin();
            }
    ...
    fn compute_spectral_centroid(&self, spectrum: &[f64], sample_rate: u32) -> f64 {
    ...
    fn compute_spectral_rolloff(&self, spectrum: &[f64], sample_rate: u32, percentile: f64) -> f64 {
        ...
        for (i, &magnitude) in spectrum.iter().enumerate() {
            ...
            if cumulative_energy >= target_energy {
                return i as f64 * sample_rate as f64 / (2.0 * spectrum.len() as f64);
            }
        }
