...
impl HierarchicalPrototypeMatcher {
    ...
    pub fn new(samples: &[ReferenceSample], normalizer: FeatureNormalizer) -> Self {
        ...
        for sample in samples {
            species_groups.entry(sample.label.clone()).or_default().push(sample);
        }

        // Build species prototypes (centroids)
        let mut prototypes_by_group: HashMap<TaxonomicGroup, Vec<SpeciesPrototype>> = HashMap::new();
    ...
    pub fn predict(&self, query: &Vector45D) -> (String, TaxonomicGroup, f32) {
        ...
        let confidence = 1.0 / (1.0 + query.weighted_distance_to(&best_match.centroid, &self.feature_weights));
    ...
    pub fn predict_flat(&self, query: &Vector45D) -> (String, TaxonomicGroup, f32) {
        ...
        (best_match.species_name.clone(), best_match.taxonomic_group, confidence)
    }

    /// Get prototype count by group
    pub fn prototype_stats(&self) -> HashMap<TaxonomicGroup, usize> {
        self.prototypes_by_group.iter().map(|(k, v)| (*k, v.len())).collect()
    }
}
...
impl LatentPrototypeMatcher {
    ...
    pub fn predict(&self, features: &[f32; 45]) -> (String, TaxonomicGroup, f32) {
        ...
        let confidence = 1.0 / (1.0 + Self::latent_distance(query_latent, &best_match.latent_vector).sqrt());
...
impl FeatureExtractor {
    ...
    fn compute_spectrum(&self, audio: &[f32]) -> Vec<f32> {
        ...
        for i in 0..window_len {
            let window = 0.5 * (1.0 - (2.0 * std::f32::consts::PI * i as f32 / window_len as f32).cos());
    ...
    fn extract_f0(&self, spectrum: &[f32]) -> (f32, f32) {
        ...
        let f0_range = if peaks.len() > 1 {
            let max_hz = peaks.iter().map(|(i, _)| *i as f32 * bin_hz).fold(0.0f32, f32::max);
            let min_hz = peaks.iter().map(|(i, _)| *i as f32 * bin_hz).fold(f32::MAX, f32::min);
            max_hz - min_hz
        } else {
            100.0
        };
    ...
    fn extract_envelope(&self, audio: &[f32]) -> (f32, f32, f32) {
        ...
        for i in 0..audio.len() {
            ...
            let avg: f32 = audio[start..end].iter().map(|x| x.abs()).sum::<f32>() / (end - start) as f32;
            envelope.push(avg);
        }

        // Find peak
        let max_val = envelope.iter().cloned().fold(0.0f32, f32::max);
        let peak_idx = envelope.iter().position(|&x| (x - max_val).abs() < 1e-10).unwrap_or(0);
        ...
        let sustain_level = if sustain_start < sustain_end && sustain_end <= envelope.len() {
            envelope[sustain_start..sustain_end].iter().sum::<f32>() / (sustain_end - sustain_start) as f32 / max_val
        } else {
            0.5
        };
...
impl RandomForestModel {
    ...
    fn load(path: &Path) -> Result<Self> {
        if path.exists() {
            ...
            println!("   Trees: {}, Classes: {}", model.trees.len(), model.n_classes);
            Ok(model)
        } else {
...
impl RosettaNet {
    ...
    fn with_prototypes(reference_samples: &[ReferenceSample]) -> Self {
        ...
        for sample in reference_samples {
            ...
            let label_entry = label_sums.entry(sample.label.clone()).or_insert(vec![0.0; 45]);
    ...
    fn predict_latent(&self, features: &[f32; 45]) -> (String, f32) {
        ...
        for (label, prototype) in &self.latent_prototypes {
            let dist: f32 = latent.iter().zip(prototype.iter()).map(|(a, b)| (a - b).powi(2)).sum();
...
impl TrainedRosettaNet {
    ...
    fn forward_with_latent(&self, input: &[f32; 45]) -> Vec<f32> {
        ...
        let normalized: Vec<f32> = if !self.feature_means.is_empty() && !self.feature_stds.is_empty() {
            input
                .iter()
        ...
        for (i, row) in self.encoder_weights.iter().enumerate().take(self.hidden_dim) {
    ...
    fn to_rosetta_net(&self) -> RosettaNet {
        ...
        let weights_layer2: Vec<Vec<f32>> = (0..self.hidden_dim)
            .map(|i| (0..self.hidden_dim).map(|j| if i == j { 1.0 } else { 0.0 }).collect())
            .collect();
...
impl ReferenceDatabase {
    ...
    fn knn_classify(&self, query: &Vector45D, k: usize) -> (String, f32, TaxonomicGroup) {
        let classification_samples: Vec<_> = self.samples.iter().filter(|s| s.task == "classification").collect();
        ...
        let none_samples: Vec<_> = classification_samples.iter().filter(|s| s.label == "None").collect();
        let species_samples: Vec<_> = classification_samples.iter().filter(|s| s.label != "None").collect();
        ...
        let mut initial_distances: Vec<_> = species_samples
            .iter()
            .map(|s| (*s, query.weighted_distance_to(&s.features, &self.feature_weights)))
            .collect();
        initial_distances.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal));
        ...
        if none_distance < none_threshold {
            ("None".to_string(), 1.0 / (1.0 + none_distance), TaxonomicGroup::Unknown)
        } else {
    ...
    fn detect(&self, query: &Vector45D, threshold: f32) -> (bool, String, f32) {
        let detection_samples: Vec<_> = self.samples.iter().filter(|s| s.task == "detection").collect();
...
fn print_comparison_table(stats: &HashMap<String, DatasetStats>) {
    let columns = [
        "watkins", "cbi", "humbugdb", "dcase", "enabirds", "hiceas", "rfcx", "gibbons", "inat", "other",
    ];
    ...
    println!(
        "║                   PER-DATASET ACCURACY COMPARISON (Zero-Shot Species Classification)                 ║"
    );
    println!("╠═══════════════╦════════════╦════════════╦════════════╦════════════╦════════════╦═════════════╣");
...
fn run_evaluation(manifest_path: &Path, limit: Option<usize>) -> Result<EvaluationResults> {
    ...
    let rf_model = RandomForestModel::load(&rf_model_path).unwrap_or_else(|e| {
        println!("   Warning: Failed to load RF model: {}, using heuristic rules", e);
    ...
    let ref_features: Vec<Vector45D> = processed.iter().take(split_idx).map(|(f, _, _, _)| f.clone()).collect();
    ...
    let hierarchical_matcher = HierarchicalPrototypeMatcher::new(&reference_db.samples, normalizer.clone());
    ...
    for (features, reference_label, task, source_dataset) in processed.iter().skip(split_idx) {
        ...
        match task.as_str() {
            "classification" => {
                ...
                // --- k-NN prediction ---
                let (knn_pred, _confidence, taxonomic_group) = reference_db.knn_classify(features, 5);
                ...
                let (latent_pred, latent_group, _latent_conf) = latent_matcher.predict(&feature_array);
    ...
    if taxonomic_total > 0 {
        ...
        println!("╠═════════════════════════════╬═════════════════╬═══════════════════╬═════════════════════════════╣");
        println!(
            "║  k-NN (Raw 45D Search)      ║    {:>8.2}%    ║     {:>8.2}%     ║  Baseline                   ║",
            classification_results.accuracy * 100.0,
            tax_acc
        );
        println!(
            "║  Hierarchical Prototypes    ║    {:>8.2}%    ║     {:>8.2}%     ║  45D centroids              ║",
            hier_species_acc, hier_taxon_acc
        );
        println!(
            "║  Latent Space Prototypes    ║    {:>8.2}%    ║     {:>8.2}%     ║  🏆 128D neural embeddings  ║",
            latent_species_acc, latent_taxon_acc
        );
        println!("╠═════════════════════════════╬═════════════════╬═══════════════════╬═════════════════════════════╣");
        println!(
            "║  Latent vs k-NN (Species)   ║    {:+>8.2}%    ║                   ║                             ║",
            latent_species_acc - (classification_results.accuracy * 100.0) as f64
        );
        println!(
            "║  Latent vs k-NN (Taxonomic) ║                 ║    {:+>8.2}%     ║                             ║",
    ...
    if classification_results.total > 0 {
        classification_results.accuracy = classification_results.correct as f32 / classification_results.total as f32;
    ...
    if detection_results.total > 0 {
        detection_results.precision = if detection_results.true_positives + detection_results.false_positives > 0 {
        ...
        detection_results.recall = if detection_results.true_positives + detection_results.false_negatives > 0 {
            detection_results.true_positives as f32
    ...
    if captioning_results.total > 0 {
        captioning_results.average_similarity =
            captioning_results.samples.iter().map(|s| s.similarity).sum::<f32>() / captioning_results.total as f32;
    }

    println!("Evaluation completed in {:.2}s", eval_start.elapsed().as_secs_f64());
    ...
    let results = EvaluationResults {
        ...
        hierarchical_taxonomic_correct,
...
fn main() -> Result<()> {
    ...
    println!("Accuracy: {:.2}%", results.classification_results.accuracy * 100.0);

    println!("\n--- Detection Results ---");
    println!("Total: {}", results.detection_results.total);
    println!("True Positives: {}", results.detection_results.true_positives);
    println!("False Positives: {}", results.detection_results.false_positives);
    println!("True Negatives: {}", results.detection_results.true_negatives);
    println!("False Negatives: {}", results.detection_results.false_negatives);
    println!("Precision: {:.2}%", results.detection_results.precision * 100.0);
    println!("Recall: {:.2}%", results.detection_results.recall * 100.0);
    println!("F1 Score: {:.2}%", results.detection_results.f1_score * 100.0);
    ...
    println!(
        ...
        (results.hierarchical_species_accuracy - results.classification_results.accuracy as f64) * 100.0
    );
    println!(
    ...
    if results.latent_species_accuracy > results.classification_results.accuracy as f64 + 0.05 {
        ...
        println!("   solving the Duration Trap by learning that 2s Frog ≠ 2s Whale.");
    } else if results.hierarchical_species_accuracy > results.classification_results.accuracy as f64 + 0.05 {
