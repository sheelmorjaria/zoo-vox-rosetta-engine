//! Train Neural Network (112D Features) using GPU (tch-rs/LibTorch)
//! ===============================================================
//!
...
//!   cargo run --release --bin train_nn_112d_gpu --features gpu-training
...
// =============================================================================
// Neural Network Definition (with LayerNorm - avoids BatchNorm requires_grad issues)
// =============================================================================
...
impl nn::Module for RosettaNet {
    fn forward(&self, xs: &Tensor) -> Tensor {
        ...
        let x = xs.apply(&self.fc1).apply(&self.ln1).gelu("none").dropout(DROPOUT, true);

        // Block 2: Linear -> LN -> GELU -> Dropout
        let x = x.apply(&self.fc2).apply(&self.ln2).gelu("none").dropout(DROPOUT, true);

        // Block 3: Linear -> LN -> GELU -> Dropout
        let x = x.apply(&self.fc3).apply(&self.ln3).gelu("none").dropout(DROPOUT, true);

        // Block 4: Linear -> LN -> GELU -> Dropout
        let x = x.apply(&self.fc4).apply(&self.ln4).gelu("none").dropout(DROPOUT, true);
...
fn main() -> Result<()> {
    println!("╔═══════════════════════════════════════════════════════════════════╗");
    println!("║  GPU Neural Network Training (112D - Improved Architecture)      ║");
    println!("╚═══════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    println!("  Hidden 4:   {}D (LN + GELU + Dropout {})", HIDDEN_DIM_4, DROPOUT);
    ...
    for label in &all_labels {
        let idx = *label_to_idx.get(label).unwrap_or(&0) as usize;
        class_counts[idx] += 1;
    }
    ...
    println!("  Class weights: min={:.2}, max={:.2}", min_weight, max_weight);
    ...
    let train_data: Vec<f32> = train_indices
        .iter()
        .flat_map(|&i| normalized_features[i].clone())
        .collect();
    ...
    let class_weights_tensor = Tensor::from_slice(&class_weights).to(device);
    ...
    for epoch in 0..EPOCHS {
        ...
        for batch in 0..n_batches {
            ...
            let end = std::cmp::min(start + BATCH_SIZE, train_size as i64);

            let batch_x = shuffled_x.narrow(0, start, end - start);
            let batch_y = shuffled_y.narrow(0, start, end - start);

            // Forward pass
            let logits = net.forward(&batch_x);

            // Standard cross-entropy loss (no class weights - they caused instability)
        ...
        if val_acc > best_val_acc {
            ...
            // Save best model
            vs.save("rosetta_net_112d_gpu.ot")?;
        } else {
            patience_counter += 1;
        }
    ...
    println!("\nSaved best model to: rosetta_net_112d_gpu.ot");
