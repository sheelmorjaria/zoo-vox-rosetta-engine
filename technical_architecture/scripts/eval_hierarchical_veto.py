...
def map_species_to_taxon(species):
    ...
    for taxon, keywords in TAXON_MAP.items():
        for keyword in keywords:
            if keyword in s:
                return taxon
    if 'gibbon' in s: return 'Mammal'
    if 'dcase' in s or 'bird' in s: return 'Songbird'
    if 'watkins' in s: return 'Cetacean'
    if 'humbug' in s or 'mosquito' in s: return 'Insect'
    if 'rfcx' in s: return 'Mammal'
    return 'Unknown'
...
def apply_veto(gatekeeper_taxon, gatekeeper_conf, nn_top5_labels, nn_top5_conf, min_gatekeeper_conf=0.5):
    """
...
def main():
    ...
    print(f"  2. Veto Applications: {veto_count} / {len(X_test)} ({veto_count/len(X_test)*100:.1f}%)")
