...
def extract_112d_features(audio_array, sample_rate=44100):
    ...
    try:
        ...
        try:
            ...
            features[idx:idx+4] = [
                np.mean(np.abs(harmonic)), np.std(np.abs(harmonic)),
                np.mean(np.abs(percussive)), np.std(np.abs(percussive))
            ]
        except:
            pass
        idx += 4
...
def main():
    ...
    parser.add_argument("--n-workers", "-w", type=int, default=mp.cpu_count(),
    ...
    print(flush=True)
    print("╔═══════════════════════════════════════════════════════════════════╗", flush=True)
    print(f"║  [Phase 1] Extracting 112D Features ({args.n_workers} workers)          ║", flush=True)
    print("╚═══════════════════════════════════════════════════════════════════╝", flush=True)
    print(flush=True)
    ...
    print(f"Extracting features from {n_samples} samples with {args.n_workers} workers...", flush=True)
    ...
    elapsed = time.time() - start_time
    print(f"Feature extraction completed in {elapsed:.1f}s ({n_samples/elapsed:.1f} samples/s)", flush=True)
    ...
    print(f"Class imbalance ratio: {max_count / max(min_count, 1):.1f}:1 (max:{max_count}, min:{min_count})", flush=True)
