//! Within-Call Phrase Discovery for Meerkat Vocalizations
//!
//! Analyzes the internal structure of individual meerkat vocalizations to discover:
...
//!
//! Dataset: /mnt/c/Users/sheel/Desktop/data/MeerKAT_10s_2024-06-12
//! Sample rate: 8kHz (meerkat vocalizations are in human-audible range)
//! Files: 384,592 WAV files (10-second clips)
...
// =============================================================================
// Configuration - Meerkat specific (8kHz)
// =============================================================================
...
impl PhraseSegmenter {
    ...
    pub fn segment(&self, audio: &[f32]) -> Vec<PhraseCandidate> {
        ...
        // Compute RMS energy in windows (10ms for 8kHz - lower resolution)
        let window_samples = (sample_rate * 0.010) as usize;
        ...
        let min_phrase_windows = (self.config.min_phrase_ms / 10.0) as usize; // 10ms windows
        let min_gap_windows = (self.config.min_gap_ms / 10.0) as usize;
...
// =============================================================================
// Feature Extraction (30D - simplified for 8kHz)
// =============================================================================
...
pub fn extract_30d_features(audio: &[f32], sample_rate: u32) -> Vec<f64> {
    ...
    // 7-9: Spectral centroid (adapted for 8kHz)
    let window_size = 256.min(n / 4).max(32);
    if n >= window_size * 2 {
    ...
    if envelope.len() > 1 {
        ...
        if max_env > 0.0 {
            ...
            let attack_samples = (attack_end - attack_start) * 50;
    ...
    if n > 100 {
        let mod_period = (sr / 20.0) as usize; // 20Hz modulation rate
    ...
    // 20-24: Fundamental frequency (adapted for 8kHz - 200Hz to 2kHz range)
    if n > 128 {
        let min_period = (sr / 2000.0) as usize; // Max 2kHz
        let max_period = (sr / 200.0).min(n as f64 / 2.0) as usize; // Min 200Hz
...
fn load_wav_file(path: &Path) -> Result<(Vec<f32>, u32), Box<dyn std::error::Error>> {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔═══════════════════════════════════════════════════════════════╗");
    println!("║   Meerkat Within-Call Phrase Discovery (8kHz)                 ║");
    println!("╠════════════════════════════════════════════════════════════════╣");
    println!("║                                                                 ║");
    println!("║  🐱 Analyzes INTERNAL structure of meerkat vocalizations      ║");
    println!("║  📊 Discovers repeated phrases, motifs, and \"syntax\"           ║");
    println!("║  🎯 Uses SIMILARITY THRESHOLDING (not clustering)              ║");
    println!("║  🔊 Optimized for 8kHz meerkat recordings                      ║");
    println!("║                                                                 ║");
    println!("╚═══════════════════════════════════════════════════════════════╝");
    println!();
    ...
    println!("Searching for WAV files in {:?}...", data_dir);
    let files = find_wav_files(&data_dir);
    ...
    // Save results
    let output_path = output_dir.join("meerkat_within_call_analyses.json");
    ...
    let summary_path = output_dir.join("meerkat_within_call_summary.json");
