...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if resume {
        if let Ok(checkpoint_content) = fs::read_to_string(&checkpoint_path) {
            if let Ok(checkpoint) = serde_json::from_str::<CheckpointData>(&checkpoint_content) {
                all_features = checkpoint.all_features.into_iter().map(|f| ExtractedFeatures {
                ...
                all_file_names = checkpoint.all_file_names;

                skip_files = discover_flac_files(&vocalizations_dir)?
                    .into_iter()
                    .filter(|path, _)| {
                        !all_file_names.iter().any(|existing| {
                            existing == path.file_name().and_then(|n| n.to_str()).unwrap_or("")
                        })
                    })
                    .collect();

                println!("   📂 Checkpoint found from {}", checkpoint.display());
                println!("   📊 Remaining files to process: {}", skip_files.len());
            }
        }
    }
