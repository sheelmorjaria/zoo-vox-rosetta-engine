// Test DBSCAN Clustering on Small Subset
//
// This example tests the clustering implementation on a small subset of files
// to verify that phrase discovery works correctly.
//
// Usage:
...
const CHECKPOINT_DIR: &str = "/tmp/bat_test_checkpoint";
const BATCH_SIZE: usize = 100; // Process 100 files per batch
const MAX_FILES: usize = 1000; // Process 1000 files total
const DBSCAN_EPS: f64 = 1.0; // Smaller eps for more diverse clusters
const MIN_SAMPLES: usize = 3; // Lower min_samples for small dataset

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔═══════════════════════════════════════════════════════════════════════════╗");
    println!("║          Test DBSCAN Clustering on Small Subset (1000 files)              ║");
    println!("╚═══════════════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    let (clustered_phrases, _vocalization_results) = batch_process_and_cluster(
        audio_dir,
        BATCH_SIZE,
        DBSCAN_EPS,
        MIN_SAMPLES,
        Path::new(CHECKPOINT_DIR),
        Some(MAX_FILES), // Process 1000 files
    )?;
