    ...
    fn increment(&self) -> usize {
        ...
        *current += 1;
        ...
        if count % 1000 == 0 || count == self.total {
            ...
            println!("  Progress: {}/{} ({:.1}%) | {:.1} items/sec | ETA: {:.1}s",
                     count, self.total, count as f64 / self.total as f64 * 100.0, rate, remaining);
        }
...
fn segment_vocalization(
        ...
        .zip(segmentation.phrase_durations_ms.iter()).enumerate()
    {
        ...
        if phrase_audio.len() < sample_rate as usize / 100 { // Less than 10ms
            continue;
        }
...
fn discover_vocabulary(
    ...
    println!("  📦 Large dataset, using chunked processing (chunk_size = {})...", CHUNK_SIZE);
    ...
    for (chunk_idx, chunk) in phrases.chunks(CHUNK_SIZE).enumerate() {
        ...
        let chunk_vocabulary = cluster_phrases_single_pass(
            chunk,
...
fn create_vocabulary_from_components(
    ...
    centroids.into_iter().zip(members).zip(contexts).zip(sources)
        .enumerate()
        .map(|(word_id, (((centroid, member_phrases), contexts), source_vocalizations))| {
...
fn analyze_reuse_patterns(
    ...
    for word in vocabulary {
        ...
        if num_contexts >= 2 {
            // General purpose word (Grammar/Syntax)
            let context_names: Vec<String> = word.contexts.iter()
            ...
            // Context specific word (Meaning/Content)
            let context_name = word.contexts.iter()
                .next()
                .map(|c| c.name().to_string())
    ...
    for (call_type, &total_phrases) in &phrases_per_context {
        ...
        let context_words: Vec<_> = vocabulary.iter()
            .filter(|w| w.contexts.contains(call_type))
            .collect();

        let unique_words = context_words.len();

        let general_purpose = context_words.iter()
            .filter(|w| w.contexts.len() >= 2)
            .count();

        let context_specific = context_words.iter()
            .filter(|w| w.contexts.len() == 1)
            .count();

        // Find most frequent words in this context
        let mut word_freq: Vec<(usize, usize)> = context_words.iter()
            .map(|w| (w.word_id, w.occurrence_count))
            .collect();
        word_freq.sort_by(|a, b| b.1.cmp(&a.1));

        context_statistics.insert(context_name.to_string(), ContextStatistics {
...
fn analyze_sequential_patterns(
    phrases: &[PhraseCandidate],
) -> SequentialAnalysis {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  ✅ Extracted {} phrase candidates from {} vocalizations",
             phrases.len(), all_files.len());
    println!();
    ...
    let vocabulary = if skip_clustering {
        ...
        let min_cluster_size_val = min_cluster_size.unwrap_or_else(|| {
            (n_total as f64).sqrt() as usize
        });
