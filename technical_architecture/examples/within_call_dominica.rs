// Within-Call Phrase Discovery for Dominica Sperm Whale Dataset
//
...
fn main() -> Result<(), Box<dyn Error>> {
    println!("╔═══════════════════════════════════════════════════════════════════════════╗");
    println!("║   Within-Call Phrase Discovery: Dominica Sperm Whale Dataset             ║");
    println!("╚═══════════════════════════════════════════════════════════════════════════╝");
    println!();

    let data_dir = Path::new("/home/sheel/birdsong_analysis/data/Dominica_dataset/Signal_parts");
    ...
    // Sperm whale click detection parameters
    let config = SegmentationConfig {
        min_phrase_ms: 10.0,    // 10ms minimum for clicks
        max_phrase_ms: 500.0,   // 500ms maximum
        energy_threshold: 0.03, // Lower threshold for clicks
        min_gap_ms: 50.0,       // 50ms minimum gap between clicks
        sample_rate: 156250,    // Will be overwritten by actual rate
    };
    let similarity_threshold = 0.70;

    let mut file_analyses = Vec::new();
    let mut total_phrases = 0;
    let mut all_types: std::collections::HashSet<i32> = std::collections::HashSet::new();
    ...
    println!("╠═══════════════════════════════════════════════════════════════════════════╣");
    println!("║                                                                           ║");
    println!("║  Dataset: Dominica Sperm Whale Recordings (156.25kHz)                    ║");
    println!("║                                                                           ║");
    println!("╠═══════════════════════════════════════════════════════════════════════════╣");
    println!("║  Statistics:                                                              ║");
    ...
    let results = DatasetResults {
        dataset: "dominica_sperm_whale".to_string(),
        ...
        file_analyses,
        summary: format!(
            "Analyzed {} Dominica sperm whale recordings, found {} phrases across {} types",
            n_files,
            total_phrases,
            all_types.len()
        ),
    ...
    let output_path = output_dir.join("dominica_within_call.json");
