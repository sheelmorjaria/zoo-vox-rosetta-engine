...
def generate_synthetic_bat_vocalization(context: int, sample_rate: int = 48000, duration_ms: float = 100.0) -> np.ndarray:
...
def extract_features_rust(audio: np.ndarray, sample_rate: int = 48000, feature_dim: str = "37d") -> Optional[np.ndarray]:
    ...
    try:
        ...
        if feature_dim == "15d":
            ...
            if hasattr(features, 'to_dict'):
                d = features.to_dict()
                return np.array([
                    d['attack_time_ms'], d['decay_time_ms'], d['sustain_level'],
                    d['vibrato_rate_hz'], d['vibrato_depth'], d['jitter'],
                    d['shimmer'], d['harmonicity'], d['spectral_flatness'],
                    d['harmonic_to_noise_ratio'], d['spectral_flux'],
                    *d['mfcc'],
                    d['median_ici_ms'], d['onset_rate_hz'], d['ici_coefficient_of_variation'],
                    100.0, 0.0, 0.0  # duration_ms, f0_mean, f0_std placeholders
                ])
            ...
            if hasattr(features, 'to_dict'):
                ...
                feature_array = np.array([
                    base['attack_time_ms'], base['decay_time_ms'], base['sustain_level'],
                    base['vibrato_rate_hz'], base['vibrato_depth'], base['jitter'],
                    base['shimmer'], base['harmonicity'], base['spectral_flatness'],
                    base['harmonic_to_noise_ratio'], base['spectral_flux'],
                    *base['mfcc'],
                    base['median_ici_ms'], base['onset_rate_hz'], base['ici_coefficient_of_variation'],
                    100.0, 0.0, 0.0,  # duration_ms, f0_mean, f0_std placeholders
                    phy['pitch_entropy'], phy['spectral_tilt'], phy['harmonic_deviation'],
                    phy['formant_f1'], phy['formant_f2'], phy['formant_f3'],
                    phy['fm_depth_hz'], phy['roughness']
                ])
...
def get_feature_names(feature_dim: str = "37d") -> List[str]:
    ...
    if feature_dim == "15d":
        ...
        return [
            ...
            "harmonicity", "spectral_flatness", "harmonic_to_noise_ratio",
            "spectral_flux",
            *[f"mfcc_{i+1}" for i in range(13)],
            "median_ici_ms", "onset_rate_hz", "ici_coefficient_of_variation",
            "duration_ms", "f0_mean", "f0_std"
        ]
    elif feature_dim == "37d":
        ...
        phylogenetic = [
            "pitch_entropy", "spectral_tilt", "harmonic_deviation",
            "formant_f1", "formant_f2", "formant_f3",
            "fm_depth_hz", "roughness"
        ]
...
def run_rfe_analysis(X: np.ndarray, y: np.ndarray, feature_names: List[str],
                    n_features_to_select: int = 15) -> Dict:
    ...
    results = {
        ...
        'feature_importances': {name: float(imp) for name, imp in zip(feature_names, full_importances)},
        ...
        'cv_scores_by_n_features': {
            f'n_{i}': float(score) for i, score in enumerate(cv_scores)
        },
...
def generate_report(results: Dict, feature_dim: str = "37d"):
    ...
    PHYLOGENETIC_FEATURES = {
        'pitch_entropy', 'spectral_tilt', 'harmonic_deviation',
        'formant_f1', 'formant_f2', 'formant_f3', 'fm_depth_hz', 'roughness'
    }
...
"""
    ...
    for i, feat in enumerate(selected_features, 1):
        ...
        if feat in PHYLOGENETIC_FEATURES:
            ...
            feat_type = "MFCC"
        elif "vibrato" in feat or "jitter" in feat or "shimmer" in feat or "attack" in feat or "decay" in feat or "sustain" in feat:
...
"""
    ...
    bird_top_features = [
        'hnr', 'formant_f2', 'fm_depth_hz', 'mfcc_1', 'sustain_level',
        'vibrato_depth', 'formant_f3', 'mfcc_2', 'spectral_flatness',
        'decay_time_ms', 'harmonic_deviation', 'shimmer', 'formant_f1',
        'mfcc_13', 'spectral_tilt'
    ]
