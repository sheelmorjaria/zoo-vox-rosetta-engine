...
class MiniBatchBGMMTeacher:
    ...
    def fit(self, features_112d: np.ndarray) -> Tuple[np.ndarray, int]:
        ...
        print(
            f"  Teacher: Discovered {self.vocabulary_size} clusters (pruned from {self.max_clusters})"
        )
...
def compute_cluster_stats(
    features_112d: np.ndarray, labels: np.ndarray, centroids: np.ndarray
) -> Dict[int, Dict]:
...
def main():
    ...
    # Configuration
    feature_path = Path(
        "/mnt/c/Users/sheel/Desktop/data/egyptian_fruit_bats/extraction_112d/extraction_112d_no_cluster.json"
    )
    ...
    teacher = MiniBatchBGMMTeacher(
        pca_components=30, max_clusters=MAX_CLUSTERS, sample_size=SAMPLE_SIZE, random_state=42
    )
    ...
    for cluster_id in sorted(cluster_stats.keys()):
        ...
        print(
            f"  {cluster_id:<6} {stats['count']:<10,} {stats['mean_distance']:<12.3f} {stats['std_distance']:<12.3f}"
        )
    print()
