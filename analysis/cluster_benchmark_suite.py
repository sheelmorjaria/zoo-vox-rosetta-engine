...
class ClusterBenchmarkSuite:
    ...
    def _fit_pca_bgmm(self, features: np.ndarray) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        ...
        print(f"  Fitting Bayesian GMM on {n_components}D data (n_components={n_bgmm_components})...")
        bgmm = BayesianGaussianMixture(
    ...
    def _fit_hnsw_hdbscan(self, features: np.ndarray) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        ...
        print(f"  Building sparse distance matrix...")
    ...
    def _fit_hnsw_leiden(self, features: np.ndarray) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        ...
        print(f"  Building igraph for Leiden clustering...")
    ...
    def _compute_metrics(
        ...
        clustered_sequences = self._apply_cluster_labels_to_sequences(
            sequences, result.labels
        )
        ...
        if NGRAM_STATS_AVAILABLE:
            try:
                ...
                lrn_depth = ngram_stats.max_ngram_length if hasattr(ngram_stats, 'max_ngram_length') else 0
            except:
                svs = 0.0
                lrn_depth = 0
        else:
        ...
        neighborhood_consistency = self._calculate_neighborhood_consistency(
            features, result.labels
        )

        avg_kl_div = 0.0
        if result.soft_labels is not None:
            avg_kl_div = self._calculate_neighbor_kl_divergence(
                features, result.soft_labels
            )
    ...
    def _compare_methods(self, results: Dict[str, BenchmarkMetrics]):
        ...
        for metrics in results.values():
            print(f"{metrics.method_name:<15} "
        ...
        print(f"  Best neighborhood: {best_consistency.method_name} ({best_consistency.neighborhood_consistency:.3f})")
