...
class GradedClusteringPipeline:
    ...
    def build_results(
        ...
        for i, (meta, label, soft_prob) in enumerate(zip(metadata, labels, soft_clusters)):
            ...
            result = GradedClusterResult(
                ...
                is_graded=is_graded,
                umap_coordinates=self.umap_embedding[i].tolist() if self.umap_embedding is not None else None
            )
    ...
    def build_cluster_profiles(self, features: np.ndarray) -> Dict[int, ClusterProfile]:
        ...
        for cluster_id, segments in cluster_segments.items():
            ...
            prob_mass = sum(
                r.cluster_probabilities.get(cluster_id, 0.0)
                for _, r in segments
            )

            # Find boundary segments (graded)
            boundary_segs = [
                idx for idx, r in segments
                if r.is_graded and cluster_id in r.cluster_probabilities
        ...
        # Print top clusters
        top_clusters = sorted(
            self.cluster_profiles.values(),
            key=lambda x: x.n_segments,
            reverse=True
        )[:10]
        ...
        for i, profile in enumerate(top_clusters):
            ...
            print(f"    {i+1}. Cluster {profile.cluster_id}: "
    ...
    def export_results(self, output_path: str) -> None:
        ...
        output_data = {
            ...
            'cluster_profiles': {
                str(k): asdict(v) for k, v in self.cluster_profiles.items()
            },
            'segments': []
        }
...
def main():
    ...
    print(f"\nNext steps:")
    print(f"  1. Analyze graded boundaries between clusters")
    print(f"  2. Run PCFG analysis on primary cluster sequences")
    print(f"  3. Investigate soft-cluster transitions for graded syntax")
