...
def analyze_vocalizations_for_ascending_syntax(
    ...
    max_vocalizations: int = MAX_VOCALIZATIONS
) -> Dict:
    ...
    results = {
        ...
        'context_patterns': defaultdict(lambda: {
            'total': 0,
            'multi_phrase': 0,
            'ascending': 0,
            'flat_ascending': 0
...
def print_ascending_syntax_summary(results: Dict):
    ...
    print(f"   Ascending syntax detected: {results['ascending_vocalizations']} "
          f"({results['ascending_vocalizations'] / total * 100:.1f}%)")
    print(f"   Flat-tone ascending: {results['flat_tone_ascending']} "
    ...
    for context, stats in sorted(results['context_patterns'].items()):
        if stats['total'] > 0:
            ...
            print(f"      Multi-phrase: {stats['multi_phrase']} ({stats['multi_phrase']/stats['total']*100:.1f}%)")
            if stats['multi_phrase'] > 0:
                print(f"      Ascending: {stats['ascending']} ({stats['ascending']/stats['multi_phrase']*100:.1f}% of multi-phrase)")
