...
def analyze_phrase_context_patterns(phrases: Dict) -> Dict:
    ...
    for phrase_key, context_counter in list(multi_context_phrases.items())[:10]:
        ...
        contexts_str = ", ".join(
            [f"{ctx} ({count})" for ctx, count in context_counter.most_common()]
        )
    ...
    for i, ctx1 in enumerate(contexts):
        for ctx2 in contexts[i + 1 :]:
            ...
            if len(shared) > 0:
                ...
                print(
                    f"   {ctx1} ↔ {ctx2}: {len(shared)} shared phrases ({overlap_pct:.1f}% overlap)"
                )
...
def analyze_compositionality_evidence(phrases: Dict):
    ...
    if flexible_pct > 20:
        ...
        print("   - Analogy: function words (flexible) + content words (context-specific)")
    elif flexible_pct > 5:
