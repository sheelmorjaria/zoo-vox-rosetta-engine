#!/usr/bin/env python3
"""
Test Modality Detection on Real Marmoset Vocalizations

This script loads real marmoset vocalization data and tests the modality detection
...
def load_flac_file(filepath, target_sr=48000):
    ...
    if not HAS_SOUNDFILE:
        raise ImportError("soundfile library required for loading FLAC files")
...
def analyze_marmoset_vocalization(audio_file_path, num_segments=5):
    """Analyze modality of a marmoset vocalization file."""
    print(f"\n{'=' * 70}")
    ...
    try:
        phrases = analyzer.segment_phrases(audio, min_gap_ms=10, min_phrase_duration_ms=10)
...
def main():
    """Main test function."""
    print("=" * 70)
    print("REAL MARMOSET VOCALIZATION MODALITY DETECTION TEST")
    print("=" * 70)
    ...
    if len(flac_files) == 0:
        print(f"❌ No FLAC files found in {data_dir}")
        return

    print(f"\n📁 Found {len(flac_files):,} FLAC files in {data_dir}")

    # Sample a few random files for testing
    num_files_to_test = min(10, len(flac_files))
    ...
    for wav_file in test_files:
        results = analyze_marmoset_vocalization(wav_file)
        if results:
