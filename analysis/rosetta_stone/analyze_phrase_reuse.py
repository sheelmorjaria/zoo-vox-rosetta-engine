...
def match_phrase_to_library(
    ...
    for phrase_key, phrase_data in phrase_library.items():
        ...
        if library_features.get('spectral_centroid_hz', 0) > 0:
            centroid_diff = abs(features.get('spectral_centroid_hz', 0) - library_features.get('spectral_centroid_hz', 0))
...
def analyze_vocalization_for_phrases(
    audio_path: str,
    phrase_library: Dict,
    urs_analyzer: UniversalRosettaStone
) -> List[Dict]:
    """
...
def analyze_phrase_reuse_patterns(
    ...
    sample_annotations = annotations.sample(n=min(max_vocalizations, len(annotations)), random_state=42)
    ...
    for idx, row in sample_annotations.iterrows():
        ...
        segments = analyze_vocalization_for_phrases(
            str(file_path),
            phrase_library,
            urs_analyzer
        )
        ...
        if any(count > 1 for count in phrase_counts.values()):
            ...
            results['phrase_reuse_count'] += sum(count - 1 for count in phrase_counts.values() if count > 1)
...
def print_analysis_summary(results: Dict):
    ...
    print(f"   Vocalizations with phrase reuse: {results['vocalizations_with_reuse']} "
...
def main():
    ...
    saveable_results = {
        ...
        'context_phrase_patterns': {k: dict(v) for k, v in results['context_phrase_patterns'].items()}
    }
