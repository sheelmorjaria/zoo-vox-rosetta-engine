...
def load_annotations(annotations_path: str, max_per_label: int = None) -> pd.DataFrame:
    ...
    if max_per_label:
        ...
        sampled_df = df.groupby('label').apply(
            lambda x: x.sample(n=min(max_per_label, len(x)), random_state=42)
    ...
    for phrase_key, phrase_data in phrase_library.items():
        ...
        for ctx_name, count in phrase_data['contexts'].most_common():
            contexts.append({
                ...
                'count': count,
                'percentage': (count / total_context_count * 100) if total_context_count > 0 else 0
            })
