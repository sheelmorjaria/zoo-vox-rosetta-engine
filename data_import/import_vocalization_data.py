...
class DataImporter:
    ...
    def import_sentence_data(self, filepath: str, species: Species):
        ...
        logger.info(
            f"Imported {len(data.get('sentences', {}))} sentence clusters for {species.value}"
        )
    ...
    def import_grammar_rules(self, filepath: str, species: Species):
        ...
        logger.info(
            f"Imported {len(data.get('grammar_rules', {}))} grammar rules for {species.value}"
        )
    ...
    def _generate_summary(self):
        ...
        for species, species_data in self.db.species_data.items():
            logger.info(
                f"  {species.value}: {species_data.total_phrases} phrases, "
