...
class SpeciesData:
    ...
    def get_sentences_with_phrase(self, phrase_key: str) -> List[Sentence]:
        ...
        return [sentence for sentence in self.sentences
                if phrase_key in sentence.phrase_sequence]
    ...
    def export_to_json(self, filepath: str):
        ...
        for species, data in self.species_data.items():
            species_dict = {
                ...
                'modality_distribution': {mod.value: count
                                       for mod, count in data.modality_distribution.items()},
                'phrases': {},
                'sentences': [],
                'grammar_rules': [],
            }
            ...
            for phrase_key, phrase in data.phrase_library.items():
                phrase_dict = {
                    ...
                    'contexts': [
                        {'context_name': ctx.context_name, 'count': ctx.count, 'percentage': ctx.percentage}
        ...
        for species_value, species_data in data['species_data'].items():
            ...
            for phrase_key, phrase_data in species_data['phrases'].items():
                acoustic_features = AcousticFeatures(
                    ...
                    acoustic_variance=phrase_data['acoustic_features'].get('acoustic_variance', 0.0),
                    # Timbre features
                    spectral_centroid_hz=phrase_data['acoustic_features'].get('spectral_centroid_hz', 0.0),
                    spectral_slope=phrase_data['acoustic_features'].get('spectral_slope', 0.0),
                    spectral_bandwidth_hz=phrase_data['acoustic_features'].get('spectral_bandwidth_hz', 0.0),
                    ...
                    spectral_contrast=phrase_data['acoustic_features'].get('spectral_contrast', 0.0),
                    ...
                    ici_coefficient_of_variation=phrase_data['acoustic_features'].get('ici_coefficient_of_variation', 0.0),
                )
