...
class TestAcousticFeatureValidation:
    ...
    def test_extract_dominant_frequency_from_synthesized_call(
        self, synthesized_phee_call, sample_rate
    ):
    ...
    def test_validate_dominant_frequency_within_tolerance(
        self, synthesized_phee_call, natural_phee_call, validation_tolerance
    ):
        ...
        assert error < tolerance, (
            f"Dominant frequency error {error:.1f} Hz exceeds tolerance {tolerance} Hz"
        )
    ...
    def test_ghost_word_f0_is_interpolated(
        self, ghost_word_call, natural_phee_call, natural_alarm_call
    ):
...
class TestPerceptualValidation:
    ...
    def test_synthesized_has_attack_decay(self, synthesized_phee_call, sample_rate):
        ...
        assert 0.1 < peak_idx / len(envelope) < 0.9, "Peak at edge (no attack/decay)"
    ...
    def _calculate_envelope(
        self, audio: np.ndarray, sample_rate: int, window_ms: float = 5.0
    ) -> np.ndarray:
...
class TestMetadataFirstSynthesisValidation:
    ...
    def test_interpolation_produces_intermediate_features(
        self, ghost_word_call, natural_phee_call, natural_alarm_call
    ):
    ...
    def _find_closest_match(
        self, target_f0: float, target_duration: float, candidates: List[Dict]
    ) -> Dict:
...
class TestStatisticalValidation:
    ...
    def test_validation_tolerance_is_reasonable(self, validation_tolerance):
        ...
        assert validation_tolerance["duration_tolerance_ms"] >= 10, "Duration tolerance too tight"
        assert validation_tolerance["duration_tolerance_ms"] <= 30, "Duration tolerance too loose"
