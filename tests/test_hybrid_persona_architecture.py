...
def harmonic_phrases(sample_rate):
    ...
    pure_phrase = PhraseSignature(Modality.HARMONIC, pure_audio, timestamp=0.0, sample_rate=sample_rate)
    ...
    gritty_phrase = PhraseSignature(Modality.HARMONIC, gritty_audio, timestamp=0.5, sample_rate=sample_rate)
    ...
    pure2_phrase = PhraseSignature(Modality.HARMONIC, pure2_audio, timestamp=1.0, sample_rate=sample_rate)
    ...
    sharp_phrase = PhraseSignature(Modality.HARMONIC, sharp_audio, timestamp=1.5, sample_rate=sample_rate)
...
class TestHybridPersonaArchitecture:
    ...
    def test_build_vocabulary_with_personas_structure(self, rosetta_stone, harmonic_phrases):
        ...
        clusters = rosetta_stone.build_vocabulary_with_personas(
            ...
            eps=0.5,
            min_samples=1,
            enable_persona_mapping=HAS_PERSONA_SUPPORT
        )
    ...
    def test_find_phrases_by_persona(self, rosetta_stone, harmonic_phrases):
        ...
        # Find PURE phrases
        pure_matches = rosetta_stone.find_phrases_by_persona(
            clusters, 'pure', min_score=0.3
        )
    ...
    def test_find_phrases_by_persona_threshold(self, rosetta_stone, harmonic_phrases):
        ...
        matches_low = rosetta_stone.find_phrases_by_persona(
            clusters, 'pure', min_score=0.1
        )
        matches_high = rosetta_stone.find_phrases_by_persona(
            clusters, 'pure', min_score=0.5
        )
    ...
    def test_backward_compatibility_build_vocabulary(self, rosetta_stone, harmonic_phrases):
        ...
        clusters = rosetta_stone.build_vocabulary(
            harmonic_phrases,
            eps=0.5,
            min_samples=1
        )
...
class TestIntegration:
    ...
    def test_full_workflow(self, rosetta_stone, harmonic_phrases):
        ...
        matches = rosetta_stone.find_phrases_by_persona(
            clusters, 'pure', min_score=0.3
        )
    ...
    def test_cross_species_persona_consistency(self, rosetta_stone, sample_rate):
        ...
        for f0 in [5000, 7000, 9000]:
            ...
            phrase = PhraseSignature(Modality.HARMONIC, audio, timestamp=float(f0), sample_rate=sample_rate)
