...
def generate_dynamic_microharmonic_samples(
    ...
    for i in range(num_samples):
        ...
        if use_rust:
            ...
            # Decay
            if decay_samples > 0:
                envelope[sustain_end:] = np.linspace(params.get('sustain_level', 0.7), 0, decay_samples) ** 0.5
...
def plot_tsne_results(projections: np.ndarray, labels: np.ndarray, output_path: str):
    ...
    color_map = {
        'Natural': 'green',
        'Concatenative': 'blue',
        'Dynamic_Microharmonic': 'red'
    }

    marker_map = {
        'Natural': 'o',
        'Concatenative': 'o',
        'Dynamic_Microharmonic': 'x'
    }
...
def main():
    ...
    micro_dynamics_db = {
        k: v for k, v in all_micro_dynamics.items()
        if v['f0_mean'] > 1000  # Minimum valid F0 threshold
    }

    print(f"  ✅ Loaded {len(micro_dynamics_db)} phrase types with valid F0 (filtered from {len(all_micro_dynamics)} total)")
