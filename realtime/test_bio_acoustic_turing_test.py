...
def test_stimulus_controller_playback_sequence():
    ...
    sequence = controller.create_counterbalanced_sequence(num_trials=20, stimulus_ids=[f'stim_{i}' for i in range(10)])
...
def test_response_recorder_measure_latency():
    ...
    response_audio = np.concatenate([response_audio, np.sin(2 * np.pi * 8000 * np.linspace(0, 0.5, 22050)) * 0.3])
...
def test_experiment_design_add_trial():
    ...
    design.add_trial(
        trial_id=1,
        stimulus_type='concatenative',
        stimulus_id='natural_phee_001'
    )

    design.add_trial(
        trial_id=2,
        stimulus_type='granular',
        stimulus_id='granular_phee_shifted'
    )
...
def test_turing_test_hypothesis_evaluation():
    ...
    turing_test.add_phase_result('concatenative', {
        'response_rate': 0.85,
        'trials': 10
    })

    turing_test.add_phase_result('granular', {
        'response_rate': 0.82,
        'trials': 10
    })
