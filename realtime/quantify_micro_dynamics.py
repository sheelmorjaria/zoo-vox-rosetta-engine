...
def analyze_micro_dynamics(features_list: List[Dict]) -> Dict:
    ...
    for feature in ['attack_time_ms', 'decay_time_ms', 'vibrato_rate_hz',
                    'vibrato_depth', 'jitter', 'spectral_flatness', 'hnr']:
    ...
    for context in df['context'].unique():
        ...
        print(f"      Attack: {context_stats[context]['attack_time_ms']['mean']:.2f} ± {context_stats[context]['attack_time_ms']['std']:.2f} ms")
        print(f"      Vibrato: {context_stats[context]['vibrato_rate_hz']['mean']:.2f} ± {context_stats[context]['vibrato_rate_hz']['std']:.2f} Hz")
        print(f"      Jitter: {context_stats[context]['jitter']['mean']:.4f} ± {context_stats[context]['jitter']['std']:.4f}")
...
def main():
    ...
    marmoset_features = load_micro_dynamics_from_audio_library(
        MARMOSET_AUDIO_INDEX, 'marmoset'
    )
    ...
    bat_features = load_micro_dynamics_from_audio_library(
        BAT_AUDIO_INDEX, 'egyptian_bat'
    )
    ...
    species_analyses['egyptian_bat'] = {
        'analysis': bat_analysis,
        'synthesis_ranges': bat_ranges
    }
        ...
        'cross_species_comparison': {
            'marmoset_vs_bat': {
                feature: {
                    ...
                    'ratio': marmoset_analysis['overall'][feature]['mean'] / bat_analysis['overall'][feature]['mean']
                }
                for feature in ['attack_time_ms', 'decay_time_ms', 'vibrato_rate_hz',
                               'vibrato_depth', 'jitter']
            }
